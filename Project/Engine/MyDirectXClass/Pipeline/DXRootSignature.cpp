...
void DXRootSignature::CreateGraphicsRootSignature(DXCommon* dxCommon, PipelineType pipelineType) {
	...
	if (pipelineType == Primitive) {
		...
		rootParameters[3].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;                           // PixelShaderで使う
		rootParameters[3].DescriptorTable.pDescriptorRanges = descriptorRangeMaskTexture;             // Tableの中身の配列を指定
		rootParameters[3].DescriptorTable.NumDescriptorRanges = _countof(descriptorRangeMaskTexture); // Tableで利用する数
		...
		// descriptorRangeForMeshInstancingの設定
		D3D12_DESCRIPTOR_RANGE descriptorRangeForMeshInstancing[1] = {};
		descriptorRangeForMeshInstancing[0].BaseShaderRegister = 0;                      // 0から始まる t0
		descriptorRangeForMeshInstancing[0].NumDescriptors = 1;                          // 数は1つ
		descriptorRangeForMeshInstancing[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV; // SRVを使う
		descriptorRangeForMeshInstancing[0].OffsetInDescriptorsFromTableStart =
			D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;                       // Offsetを自動計算

		// descriptorRangeForTriangleInstancingの設定
		D3D12_DESCRIPTOR_RANGE descriptorRangeForTriangleInstancing[1] = {};
		descriptorRangeForTriangleInstancing[0].BaseShaderRegister = 1;                      // 0から始まる t1
		descriptorRangeForTriangleInstancing[0].NumDescriptors = 1;                          // 数は1つ
		descriptorRangeForTriangleInstancing[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV; // SRVを使う
		descriptorRangeForTriangleInstancing[0].OffsetInDescriptorsFromTableStart =
			D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;                       // Offsetを自動計算
		...
		rootParameters[1].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;                       // DescriptorTableを使う
		rootParameters[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX;                                // VertexShaderで使う
		rootParameters[1].DescriptorTable.pDescriptorRanges = descriptorRangeForMeshInstancing;             // Tableの中身の配列を指定
		rootParameters[1].DescriptorTable.NumDescriptorRanges = _countof(descriptorRangeForMeshInstancing); // Tableで利用する数

		rootParameters[2].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;                           // DescriptorTableを使う
		rootParameters[2].ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX;                                    // VertexShaderで使う
		rootParameters[2].DescriptorTable.pDescriptorRanges = descriptorRangeForTriangleInstancing;             // Tableの中身の配列を指定
		rootParameters[2].DescriptorTable.NumDescriptorRanges = _countof(descriptorRangeForTriangleInstancing); // Tableで利用する数
