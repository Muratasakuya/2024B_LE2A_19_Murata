...
*							SkinClusterを作成する関数
////////////////////////////////////////////////////////////////////////////////*/
/// 今はMultiMeshに対応していない。最初のMeshのみ適応
SkinCluster ModelManager::CreateSkinCluster(const std::string& modelName, const std::string& animationName) {
	...
	skinCluster.paletteResource =
		vertexResource.CreateBufferResource(device, sizeof(WellForGPU) * skeletons_[animationName].joints.size());
	...
	skinCluster.influenceResource = vertexResource.CreateBufferResource(device, sizeof(VertexInfluence) * models_[modelName].meshes.front().vertices.size());
...
ModelData ModelManager::LoadObjFile(const std::string& directoryPath, const std::string& filename) {
	...
	for (uint32_t meshIndex = 0; meshIndex < scene->mNumMeshes; ++meshIndex) {
		...
		if (material->GetTextureCount(aiTextureType_DIFFUSE) != 0) {
			...
			meshModelData.material.textureName = textureName.C_Str();

			std::filesystem::path name(meshModelData.material.textureName.value());
...
void ModelManager::LoadModel(const std::string& directoryPath, const std::string& modelName) {
	...
	// モデル読み込み
	ModelData modelData = LoadObjFile(directoryPath, modelName);
