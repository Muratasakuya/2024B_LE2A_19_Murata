...
template<typename T>
void DXConstBuffer<T>::SetCommand(ID3D12GraphicsCommandList* commandList, UINT rootParameterIndex) {

	commandList->SetGraphicsRootConstantBufferView(rootParameterIndex, resource_.Get()->GetGPUVirtualAddress());
}

template<typename T>
void DXConstBuffer<T>::CreateConstBuffer(ID3D12Device* device) {
