...
void Sphere::Draw(PipelineType drawType, BlendMode blendMode) {

	auto commandList = NewMoon::GetCommandList();
	DXConstBufferManager constBuffer;

	NewMoon::SetGraphicsPipeline(commandList, drawType, blendMode);
	commandList->IASetVertexBuffers(0, 1, &vertex_.GetVertexBuffer());
	commandList->IASetIndexBuffer(&index_.GetIndexBuffer());
	constBuffer.SetCommands(commandList, drawType, worldTransform_, material_, light_.front());
